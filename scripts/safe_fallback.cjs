...
if (quick.includes(qTarget)) {
    quick = quick.replace(qTarget, qReplace);
} else {
    console.log("qTarget not found in QuickMatchArena.tsx");
}

const botTarget = `if (searchTimer > 0) {
...
const botReplace = `if (searchTimer > 0) {
      interval = setTimeout(() => setSearchTimer(t => t - 1), 1000);
    } else if (searchTimer === 0 && view === 'searching') {
      setIsBot(true);
      setOppId('bot-1');
