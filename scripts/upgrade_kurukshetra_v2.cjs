...
export default function Kurukshetra({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className={\`fixed inset-0 z-50 bg-[#050510] text-white flex flex-col overflow-hidden \${shake ? 'animate-[shake_0.5s_ease-in-out]' : ''}\`}>
      ...
      <div className="relative p-6 flex justify-between items-center border-b border-amber-500/20 bg-black/50 backdrop-blur-md z-10">
      ...
      <div className="relative flex-1 overflow-y-auto p-4 md:p-8 flex flex-col items-center justify-center z-10">
        ...
        {view === 'battle' && (
          <div className="w-full max-w-5xl h-full flex flex-col gap-6 relative">
            ...
            {/* Question UI */}
            {currentQ ? (
              <div className="flex-1 bg-black/70 border border-slate-800 rounded-3xl p-8 flex flex-col shadow-2xl relative overflow-hidden backdrop-blur-md z-10">
                ...
                <div className="flex-1 overflow-y-auto mb-6 pr-4 custom-scrollbar">
                  <div className="prose prose-invert prose-xl max-w-none font-serif leading-relaxed">
...
console.log('Kurukshetra upgraded with Phase 2 Mythic Layer');
