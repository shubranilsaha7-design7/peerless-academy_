...
export default function QuickMatchArena({ onBack }: { onBack?: () => void }) {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      const id = user?.id || 'guest-' + Math.floor(Math.random()*1000);
      setUserId(id);
      if (user) {
    ...
    // Pre-fetch 5 random questions
    const fetchQ = async () => {
      const { data } = await (supabase as any).from('pyqs').select('*').limit(20);
  ...
  useEffect(() => {
    let interval: any;
    if (view === 'searching') {
      const channel = supabase.channel('quickmatch_queue');
      channel
        .on('broadcast', { event: 'looking_for_match' }, (payload) => {
          if (payload.payload.userId !== userId) {
            setOppId(payload.payload.userId);
            setOppName(payload.payload.userName);
            setIsBot(false);
            channel.send({ type: 'broadcast', event: 'match_found', payload: { userId, userName } });
        ...
        .subscribe((status) => {
          if (status === 'SUBSCRIBED') {
            channel.send({ type: 'broadcast', event: 'looking_for_match', payload: { userId, userName } });
          }
        });
  ...
  const handleAnswer = (selectedIndex: number) => {
    ...
    if (selectedIndex === currentQ.correct_option) {
      setMyScore(s => s + 4);
    } else {
